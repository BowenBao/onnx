...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .TypeAndShapeInferenceFunction([](InferenceContext& ctx) {
          propagateElemTypeFromAttributeToOutput(ctx, "dtype", 0, TensorProto::FLOAT, TypeProto::kSequenceType);
        }));
...
ONNX_OPERATOR_SET_SCHEMA(
    ...
    OpSchema()
        ...
        .TypeConstraint(
            ...
            "Constrain output types to any tensor type.")
        .TypeAndShapeInferenceFunction([](InferenceContext& ctx) {
          propagateElemTypeFromInputToOutput(ctx, 0, 0, TypeProto::kSequenceType);
          ...
          *(output_tensor_type->mutable_shape()) = ctx.getInputType(0)->tensor_type().shape();
          ...
          for (size_t i = 1; i < numInputs; ++i) {
            ...
            MergeShapeInfoToSequence(input_shape, *output_tensor_type);
          }
        }));
    ...
    OpSchema()
        ...
        .TypeAndShapeInferenceFunction([](InferenceContext& ctx) {
          ...
          *(output_tensor_type->mutable_shape()) = 
          ...
          MergeShapeInfoToSequence(ctx.getInputType(1)->tensor_type().shape(), *output_tensor_type);
        }));
